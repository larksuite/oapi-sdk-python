...
class V6(object):
    def __init__(self, config: Config) -> None:
        ...
        self.application_app_version: ApplicationAppVersion = ApplicationAppVersion(
            config
        )
        self.application_collaborators: ApplicationCollaborators = (
            ApplicationCollaborators(config)
        )
        self.application_contacts_range: ApplicationContactsRange = (
            ApplicationContactsRange(config)
        )
        ...
        self.application_owner: ApplicationOwner = ApplicationOwner(config)
        self.application_visibility: ApplicationVisibility = ApplicationVisibility(
            config
        )
