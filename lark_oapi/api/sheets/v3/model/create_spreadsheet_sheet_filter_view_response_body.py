# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.construct import init
from .filter_view import FilterView


class CreateSpreadsheetSheetFilterViewResponseBody(object):
    _types = {
        "filter_view": FilterView,
    }

    def __init__(self, d):
        self.filter_view: Optional[FilterView] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "CreateSpreadsheetSheetFilterViewResponseBodyBuilder":
        return CreateSpreadsheetSheetFilterViewResponseBodyBuilder()


class CreateSpreadsheetSheetFilterViewResponseBodyBuilder(object):
    def __init__(self, create_spreadsheet_sheet_filter_view_response_body: CreateSpreadsheetSheetFilterViewResponseBody = CreateSpreadsheetSheetFilterViewResponseBody({})) -> None:
        self._create_spreadsheet_sheet_filter_view_response_body: CreateSpreadsheetSheetFilterViewResponseBody = create_spreadsheet_sheet_filter_view_response_body
    
    def filter_view(self, filter_view: FilterView) -> "CreateSpreadsheetSheetFilterViewResponseBodyBuilder":
        self._create_spreadsheet_sheet_filter_view_response_body.filter_view = filter_view
        return self
    
    def build(self) -> "CreateSpreadsheetSheetFilterViewResponseBody":
        return self._create_spreadsheet_sheet_filter_view_response_body