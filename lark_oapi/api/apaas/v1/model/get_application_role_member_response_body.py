# Code generated by Lark OpenAPI.

from typing import Any, Optional, Union, Dict, List, Set, IO, Callable, Type
from lark_oapi.core.construct import init
from .role_member import RoleMember


class GetApplicationRoleMemberResponseBody(object):
    _types = {
        "role_member": RoleMember,
    }

    def __init__(self, d=None):
        self.role_member: Optional[RoleMember] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "GetApplicationRoleMemberResponseBodyBuilder":
        return GetApplicationRoleMemberResponseBodyBuilder()


class GetApplicationRoleMemberResponseBodyBuilder(object):
    def __init__(self) -> None:
        self._get_application_role_member_response_body = (
            GetApplicationRoleMemberResponseBody()
        )

    def role_member(
        self, role_member: RoleMember
    ) -> "GetApplicationRoleMemberResponseBodyBuilder":
        self._get_application_role_member_response_body.role_member = role_member
        return self

    def build(self) -> "GetApplicationRoleMemberResponseBody":
        return self._get_application_role_member_response_body
