# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.construct import init
from .memberlist import Memberlist


class BatchAddGroupMemberRequestBody(object):
    _types = {
        "members": List[Memberlist],
    }

    def __init__(self, d):
        self.members: Optional[List[Memberlist]] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "BatchAddGroupMemberRequestBodyBuilder":
        return BatchAddGroupMemberRequestBodyBuilder()


class BatchAddGroupMemberRequestBodyBuilder(object):
    def __init__(self, batch_add_group_member_request_body: BatchAddGroupMemberRequestBody = BatchAddGroupMemberRequestBody({})) -> None:
        self._batch_add_group_member_request_body: BatchAddGroupMemberRequestBody = batch_add_group_member_request_body
    
    def members(self, members: List[Memberlist]) -> "BatchAddGroupMemberRequestBodyBuilder":
        self._batch_add_group_member_request_body.members = members
        return self
    
    def build(self) -> "BatchAddGroupMemberRequestBody":
        return self._batch_add_group_member_request_body