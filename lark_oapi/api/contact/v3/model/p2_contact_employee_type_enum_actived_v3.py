# Code generated by Lark OpenAPI.

from typing import Any, Optional, Union, Dict, List, Set, IO, Callable, Type
from lark_oapi.core.construct import init
from lark_oapi.event.context import EventContext
from .employee_type_enum import EmployeeTypeEnum
from .employee_type_enum import EmployeeTypeEnum


class P2ContactEmployeeTypeEnumActivedV3Data(object):
    _types = {
        "old_enum": EmployeeTypeEnum,
        "new_enum": EmployeeTypeEnum,
    }

    def __init__(self, d=None):
        self.old_enum: Optional[EmployeeTypeEnum] = None
        self.new_enum: Optional[EmployeeTypeEnum] = None
        init(self, d, self._types)


class P2ContactEmployeeTypeEnumActivedV3(EventContext):
    _types = {"event": P2ContactEmployeeTypeEnumActivedV3Data}

    def __init__(self, d=None):
        super().__init__(d)
        self._types.update(super()._types)
        self.event: Optional[P2ContactEmployeeTypeEnumActivedV3Data] = None
        init(self, d, self._types)
