# Code generated by Lark OpenAPI.

from typing import *

from lark_oapi.core.construct import init


class SystemStatusSyncI18nName(object):
    _types = {
        "zh_cn": str,
        "en_us": str,
        "ja_jp": str,
    }

    def __init__(self, d):
        self.zh_cn: Optional[str] = None
        self.en_us: Optional[str] = None
        self.ja_jp: Optional[str] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "SystemStatusSyncI18nNameBuilder":
        return SystemStatusSyncI18nNameBuilder()


class SystemStatusSyncI18nNameBuilder(object):
    def __init__(self, system_status_sync_i18n_name: SystemStatusSyncI18nName = SystemStatusSyncI18nName({})) -> None:
        self._system_status_sync_i18n_name: SystemStatusSyncI18nName = system_status_sync_i18n_name

    def zh_cn(self, zh_cn: str) -> "SystemStatusSyncI18nNameBuilder":
        self._system_status_sync_i18n_name.zh_cn = zh_cn
        return self

    def en_us(self, en_us: str) -> "SystemStatusSyncI18nNameBuilder":
        self._system_status_sync_i18n_name.en_us = en_us
        return self

    def ja_jp(self, ja_jp: str) -> "SystemStatusSyncI18nNameBuilder":
        self._system_status_sync_i18n_name.ja_jp = ja_jp
        return self

    def build(self) -> "SystemStatusSyncI18nName":
        return self._system_status_sync_i18n_name
