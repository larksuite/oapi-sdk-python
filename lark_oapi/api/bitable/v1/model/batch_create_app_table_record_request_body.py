# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.construct import init
from .app_table_record import AppTableRecord


class BatchCreateAppTableRecordRequestBody(object):
    _types = {
        "records": List[AppTableRecord],
    }

    def __init__(self, d):
        self.records: Optional[List[AppTableRecord]] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "BatchCreateAppTableRecordRequestBodyBuilder":
        return BatchCreateAppTableRecordRequestBodyBuilder()


class BatchCreateAppTableRecordRequestBodyBuilder(object):
    def __init__(self, batch_create_app_table_record_request_body: BatchCreateAppTableRecordRequestBody = BatchCreateAppTableRecordRequestBody({})) -> None:
        self._batch_create_app_table_record_request_body: BatchCreateAppTableRecordRequestBody = batch_create_app_table_record_request_body
    
    def records(self, records: List[AppTableRecord]) -> "BatchCreateAppTableRecordRequestBodyBuilder":
        self._batch_create_app_table_record_request_body.records = records
        return self
    
    def build(self) -> "BatchCreateAppTableRecordRequestBody":
        return self._batch_create_app_table_record_request_body