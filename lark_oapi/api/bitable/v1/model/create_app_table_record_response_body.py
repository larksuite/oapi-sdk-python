# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.construct import init
from .app_table_record import AppTableRecord


class CreateAppTableRecordResponseBody(object):
    _types = {
        "record": AppTableRecord,
    }

    def __init__(self, d):
        self.record: Optional[AppTableRecord] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "CreateAppTableRecordResponseBodyBuilder":
        return CreateAppTableRecordResponseBodyBuilder()


class CreateAppTableRecordResponseBodyBuilder(object):
    def __init__(self, create_app_table_record_response_body: CreateAppTableRecordResponseBody = CreateAppTableRecordResponseBody({})) -> None:
        self._create_app_table_record_response_body: CreateAppTableRecordResponseBody = create_app_table_record_response_body
    
    def record(self, record: AppTableRecord) -> "CreateAppTableRecordResponseBodyBuilder":
        self._create_app_table_record_response_body.record = record
        return self
    
    def build(self) -> "CreateAppTableRecordResponseBody":
        return self._create_app_table_record_response_body