# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.construct import init
from .admin_user_stat import AdminUserStat


class ListAdminUserStatResponseBody(object):
    _types = {
        "has_more": bool,
        "page_token": str,
        "items": List[AdminUserStat],
    }

    def __init__(self, d):
        self.has_more: Optional[bool] = None
        self.page_token: Optional[str] = None
        self.items: Optional[List[AdminUserStat]] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "ListAdminUserStatResponseBodyBuilder":
        return ListAdminUserStatResponseBodyBuilder()


class ListAdminUserStatResponseBodyBuilder(object):
    def __init__(self, list_admin_user_stat_response_body: ListAdminUserStatResponseBody = ListAdminUserStatResponseBody({})) -> None:
        self._list_admin_user_stat_response_body: ListAdminUserStatResponseBody = list_admin_user_stat_response_body
    
    def has_more(self, has_more: bool) -> "ListAdminUserStatResponseBodyBuilder":
        self._list_admin_user_stat_response_body.has_more = has_more
        return self
    
    def page_token(self, page_token: str) -> "ListAdminUserStatResponseBodyBuilder":
        self._list_admin_user_stat_response_body.page_token = page_token
        return self
    
    def items(self, items: List[AdminUserStat]) -> "ListAdminUserStatResponseBodyBuilder":
        self._list_admin_user_stat_response_body.items = items
        return self
    
    def build(self) -> "ListAdminUserStatResponseBody":
        return self._list_admin_user_stat_response_body