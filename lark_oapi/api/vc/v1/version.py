...
class V1(object):
    def __init__(self, config: Config) -> None:
        ...
        self.participant_quality_list: ParticipantQualityList = ParticipantQualityList(
            config
        )
        ...
        self.reserve_config_disable_inform: ReserveConfigDisableInform = (
            ReserveConfigDisableInform(config)
        )
        self.reserve_config_form: ReserveConfigForm = ReserveConfigForm(config)
        self.resource_reservation_list: ResourceReservationList = (
            ResourceReservationList(config)
        )
