# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.model import BaseRequest
from lark_oapi.core.enum import HttpMethod, AccessTokenType


class GetReserveConfigAdminRequest(BaseRequest):
    def __init__(self) -> None:
        super().__init__()
        self.scope_type: Optional[int] = None
        self.user_id_type: Optional[str] = None
        self.reserve_config_id: Optional[str] = None

    @staticmethod
    def builder() -> "GetReserveConfigAdminRequestBuilder":
        return GetReserveConfigAdminRequestBuilder()


class GetReserveConfigAdminRequestBuilder(object):

    def __init__(self, get_reserve_config_admin_request: GetReserveConfigAdminRequest = GetReserveConfigAdminRequest()) -> None:
        get_reserve_config_admin_request.http_method = HttpMethod.GET
        get_reserve_config_admin_request.uri = "/open-apis/vc/v1/reserve_configs/:reserve_config_id/admin"
        get_reserve_config_admin_request.token_types = {AccessTokenType.TENANT, AccessTokenType.USER}
        self._get_reserve_config_admin_request: GetReserveConfigAdminRequest = get_reserve_config_admin_request
    
    def scope_type(self, scope_type: int) -> "GetReserveConfigAdminRequestBuilder":
        self._get_reserve_config_admin_request.scope_type = scope_type
        self._get_reserve_config_admin_request.queries["scope_type"] = str(scope_type)
        return self
    
    def user_id_type(self, user_id_type: str) -> "GetReserveConfigAdminRequestBuilder":
        self._get_reserve_config_admin_request.user_id_type = user_id_type
        self._get_reserve_config_admin_request.queries["user_id_type"] = str(user_id_type)
        return self
    
    def reserve_config_id(self, reserve_config_id: str) -> "GetReserveConfigAdminRequestBuilder":
        self._get_reserve_config_admin_request.reserve_config_id = reserve_config_id
        self._get_reserve_config_admin_request.paths["reserve_config_id"] = reserve_config_id
        return self
    

    def build(self) -> GetReserveConfigAdminRequest:
        return self._get_reserve_config_admin_request
