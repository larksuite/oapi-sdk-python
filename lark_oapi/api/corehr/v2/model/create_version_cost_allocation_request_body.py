# Code generated by Lark OpenAPI.

from typing import Any, Optional, Union, Dict, List, Set, IO, Callable, Type
from lark_oapi.core.construct import init
from .employment_cost_allocation import EmploymentCostAllocation


class CreateVersionCostAllocationRequestBody(object):
    _types = {
        "employment_id": str,
        "cost_allocation": EmploymentCostAllocation,
    }

    def __init__(self, d=None):
        self.employment_id: Optional[str] = None
        self.cost_allocation: Optional[EmploymentCostAllocation] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "CreateVersionCostAllocationRequestBodyBuilder":
        return CreateVersionCostAllocationRequestBodyBuilder()


class CreateVersionCostAllocationRequestBodyBuilder(object):
    def __init__(self) -> None:
        self._create_version_cost_allocation_request_body = (
            CreateVersionCostAllocationRequestBody()
        )

    def employment_id(
        self, employment_id: str
    ) -> "CreateVersionCostAllocationRequestBodyBuilder":
        self._create_version_cost_allocation_request_body.employment_id = employment_id
        return self

    def cost_allocation(
        self, cost_allocation: EmploymentCostAllocation
    ) -> "CreateVersionCostAllocationRequestBodyBuilder":
        self._create_version_cost_allocation_request_body.cost_allocation = (
            cost_allocation
        )
        return self

    def build(self) -> "CreateVersionCostAllocationRequestBody":
        return self._create_version_cost_allocation_request_body
