# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.construct import init
from .job_family import JobFamily


class PatchJobFamilyResponseBody(object):
    _types = {
        "job_family": JobFamily,
    }

    def __init__(self, d):
        self.job_family: Optional[JobFamily] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "PatchJobFamilyResponseBodyBuilder":
        return PatchJobFamilyResponseBodyBuilder()


class PatchJobFamilyResponseBodyBuilder(object):
    def __init__(self, patch_job_family_response_body: PatchJobFamilyResponseBody = PatchJobFamilyResponseBody({})) -> None:
        self._patch_job_family_response_body: PatchJobFamilyResponseBody = patch_job_family_response_body
    
    def job_family(self, job_family: JobFamily) -> "PatchJobFamilyResponseBodyBuilder":
        self._patch_job_family_response_body.job_family = job_family
        return self
    
    def build(self) -> "PatchJobFamilyResponseBody":
        return self._patch_job_family_response_body