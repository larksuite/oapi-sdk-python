# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.construct import init
from .national_id_type import NationalIdType


class PatchNationalIdTypeResponseBody(object):
    _types = {
        "national_id_type": NationalIdType,
    }

    def __init__(self, d):
        self.national_id_type: Optional[NationalIdType] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "PatchNationalIdTypeResponseBodyBuilder":
        return PatchNationalIdTypeResponseBodyBuilder()


class PatchNationalIdTypeResponseBodyBuilder(object):
    def __init__(self, patch_national_id_type_response_body: PatchNationalIdTypeResponseBody = PatchNationalIdTypeResponseBody({})) -> None:
        self._patch_national_id_type_response_body: PatchNationalIdTypeResponseBody = patch_national_id_type_response_body
    
    def national_id_type(self, national_id_type: NationalIdType) -> "PatchNationalIdTypeResponseBodyBuilder":
        self._patch_national_id_type_response_body.national_id_type = national_id_type
        return self
    
    def build(self) -> "PatchNationalIdTypeResponseBody":
        return self._patch_national_id_type_response_body