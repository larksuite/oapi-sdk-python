...
class V1(object):
    def __init__(self, config: Config) -> None:
        ...
        self.eco_account_custom_field: EcoAccountCustomField = EcoAccountCustomField(
            config
        )
        self.eco_background_check: EcoBackgroundCheck = EcoBackgroundCheck(config)
        self.eco_background_check_custom_field: EcoBackgroundCheckCustomField = (
            EcoBackgroundCheckCustomField(config)
        )
        self.eco_background_check_package: EcoBackgroundCheckPackage = (
            EcoBackgroundCheckPackage(config)
        )
        ...
        self.ehr_import_task_for_internship_offer: EhrImportTaskForInternshipOffer = (
            EhrImportTaskForInternshipOffer(config)
        )
        ...
        self.external_background_check: ExternalBackgroundCheck = (
            ExternalBackgroundCheck(config)
        )
        self.external_interview: ExternalInterview = ExternalInterview(config)
        self.external_interview_assessment: ExternalInterviewAssessment = (
            ExternalInterviewAssessment(config)
        )
        self.external_offer: ExternalOffer = ExternalOffer(config)
        self.external_referral_reward: ExternalReferralReward = ExternalReferralReward(
            config
        )
        self.interview: Interview = Interview(config)
        self.interview_feedback_form: InterviewFeedbackForm = InterviewFeedbackForm(
            config
        )
        self.interview_record: InterviewRecord = InterviewRecord(config)
        self.interview_record_attachment: InterviewRecordAttachment = (
            InterviewRecordAttachment(config)
        )
        self.interview_registration_schema: InterviewRegistrationSchema = (
            InterviewRegistrationSchema(config)
        )
        ...
        self.offer_approval_template: OfferApprovalTemplate = OfferApprovalTemplate(
            config
        )
        self.offer_custom_field: OfferCustomField = OfferCustomField(config)
        self.offer_schema: OfferSchema = OfferSchema(config)
        self.portal_apply_schema: PortalApplySchema = PortalApplySchema(config)
        ...
        self.referral_website_job_post: ReferralWebsiteJobPost = ReferralWebsiteJobPost(
            config
        )
