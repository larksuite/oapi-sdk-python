# Code generated by Lark OpenAPI.

from typing import *
from typing import IO
from lark_oapi.core.construct import init


class JobRequirementCustomizedData(object):
    _types = {
        "object_id": str,
        "value": str,
    }

    def __init__(self, d):
        self.object_id: Optional[str] = None
        self.value: Optional[str] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "JobRequirementCustomizedDataBuilder":
        return JobRequirementCustomizedDataBuilder()


class JobRequirementCustomizedDataBuilder(object):
    def __init__(self, job_requirement_customized_data: JobRequirementCustomizedData = JobRequirementCustomizedData({})) -> None:
        self._job_requirement_customized_data: JobRequirementCustomizedData = job_requirement_customized_data
    
    def object_id(self, object_id: str) -> "JobRequirementCustomizedDataBuilder":
        self._job_requirement_customized_data.object_id = object_id
        return self
    
    def value(self, value: str) -> "JobRequirementCustomizedDataBuilder":
        self._job_requirement_customized_data.value = value
        return self
    
    def build(self) -> "JobRequirementCustomizedData":
        return self._job_requirement_customized_data