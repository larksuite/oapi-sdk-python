# Code generated by Lark OpenAPI.

from typing import Any, Optional, Union, Dict, List, Set, IO, Callable, Type
from lark_oapi.core.construct import init
from .portal_job_post import PortalJobPost


class GetReferralWebsiteJobPostResponseBody(object):
    _types = {
        "job_post": PortalJobPost,
    }

    def __init__(self, d=None):
        self.job_post: Optional[PortalJobPost] = None
        init(self, d, self._types)

    @staticmethod
    def builder() -> "GetReferralWebsiteJobPostResponseBodyBuilder":
        return GetReferralWebsiteJobPostResponseBodyBuilder()


class GetReferralWebsiteJobPostResponseBodyBuilder(object):
    def __init__(self) -> None:
        self._get_referral_website_job_post_response_body = (
            GetReferralWebsiteJobPostResponseBody()
        )

    def job_post(
        self, job_post: PortalJobPost
    ) -> "GetReferralWebsiteJobPostResponseBodyBuilder":
        self._get_referral_website_job_post_response_body.job_post = job_post
        return self

    def build(self) -> "GetReferralWebsiteJobPostResponseBody":
        return self._get_referral_website_job_post_response_body
