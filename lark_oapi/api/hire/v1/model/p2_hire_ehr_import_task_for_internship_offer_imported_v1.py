# Code generated by Lark OpenAPI.

from typing import Any, Optional, Union, Dict, List, Set, IO, Callable, Type
from lark_oapi.core.construct import init
from lark_oapi.event.context import EventContext
from .user_id import UserId
from .department_id import DepartmentId


class P2HireEhrImportTaskForInternshipOfferImportedV1Data(object):
    _types = {
        "task_id": str,
        "application_id": str,
        "offer_id": str,
        "pre_onboard_id": str,
        "ehr_department_id": str,
        "operator_id": str,
        "operator_user_id": UserId,
        "ehr_department": DepartmentId,
    }

    def __init__(self, d=None):
        self.task_id: Optional[str] = None
        self.application_id: Optional[str] = None
        self.offer_id: Optional[str] = None
        self.pre_onboard_id: Optional[str] = None
        self.ehr_department_id: Optional[str] = None
        self.operator_id: Optional[str] = None
        self.operator_user_id: Optional[UserId] = None
        self.ehr_department: Optional[DepartmentId] = None
        init(self, d, self._types)


class P2HireEhrImportTaskForInternshipOfferImportedV1(EventContext):
    _types = {"event": P2HireEhrImportTaskForInternshipOfferImportedV1Data}

    def __init__(self, d=None):
        super().__init__(d)
        self._types.update(super()._types)
        self.event: Optional[P2HireEhrImportTaskForInternshipOfferImportedV1Data] = None
        init(self, d, self._types)
