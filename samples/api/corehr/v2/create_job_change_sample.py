...
def main():
    ...
    request: CreateJobChangeRequest = CreateJobChangeRequest.builder() \
        ...
        .request_body(CreateJobChangeRequestBody.builder()
                      ...
                      .transfer_info(CreateTransferInfo.builder().build())
    ...
    if not response.success():
        lark.logger.error(
            f"client.corehr.v2.job_change.create failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
        return
...
async def amain():
    ...
    # 发起请求
    response: CreateJobChangeResponse = await client.corehr.v2.job_change.acreate(request)

    # 处理失败返回
    if not response.success():
        lark.logger.error(
            f"client.corehr.v2.job_change.acreate failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
        return
