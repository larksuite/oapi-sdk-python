...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.apaas.v1.approval_task.add_assignee failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
        return
...
async def amain():
    ...
    # 发起请求
    response: AddAssigneeApprovalTaskResponse = await client.apaas.v1.approval_task.aadd_assignee(request)

    # 处理失败返回
    if not response.success():
        lark.logger.error(
            f"client.apaas.v1.approval_task.aadd_assignee failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
        return
