...
def main():
	...
	if not response.success():
		lark.logger.error(
			f"client.apaas.v1.application_object.oql_query failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
		return
...
async def amain():
	...
	# 发起请求
	response: OqlQueryApplicationObjectResponse = await client.apaas.v1.application_object.aoql_query(request)

	# 处理失败返回
	if not response.success():
		lark.logger.error(
			f"client.apaas.v1.application_object.aoql_query failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
		return
