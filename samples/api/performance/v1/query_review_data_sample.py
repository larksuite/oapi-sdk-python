...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.performance.v1.review_data.query failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
        return
...
async def amain():
    ...
    # 发起请求
    response: QueryReviewDataResponse = await client.performance.v1.review_data.aquery(request)

    # 处理失败返回
    if not response.success():
        lark.logger.error(
            f"client.performance.v1.review_data.aquery failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
        return
