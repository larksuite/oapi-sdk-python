...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.hire.v1.external_interview.update failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
        return
...
async def amain():
    ...
    # 发起请求
    response: UpdateExternalInterviewResponse = await client.hire.v1.external_interview.aupdate(request)

    # 处理失败返回
    if not response.success():
        lark.logger.error(
            f"client.hire.v1.external_interview.aupdate failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}")
        return
